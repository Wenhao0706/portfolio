...
/**
 * Keeps the `bottom-6` corner slot. The chat launcher stacks in the same column one step
 * above it, so the two read as a column of round controls rather than competing for the
 * corner.
 */
...
export default function ScrollToTop() {
  ...
  return (
    <button
      ...
      className={`group fixed bottom-6 right-6 z-40 flex h-12 w-12 items-center justify-center rounded-full ${RAISED_SURFACE} text-[#7A7568] dark:text-[#8A9099] shadow-[0_4px_16px_-4px_rgba(43,42,38,0.25)] transition-[opacity,transform,color,border-color] duration-200 cursor-pointer hover:border-[#B5772E] dark:hover:border-[#D9A441] hover:text-[#B5772E] dark:hover:text-[#D9A441] ${FOCUS_RING} ${
