...
| `react-hooks/set-state-in-effect` errors on a component that renders a client-only value (a clock, a media query, anything the server cannot know) | Reaching for `useState` plus a mount `useEffect` to dodge a hydration mismatch is the cascading-render pattern the rule exists to catch | Use `useSyncExternalStore` with a `null` `getServerSnapshot`, so the markup ships without the value and fills in on hydration — see `components/LocalTime.tsx` |
| A component throws `IntersectionObserver is not defined` or `matchMedia is not a function` in tests, though it works in the browser | jsdom implements neither, so anything building one at mount fails the moment a test renders it | `vitest.setup.ts` provides an inert IntersectionObserver globally; for `matchMedia`, guard with `typeof window.matchMedia !== 'function'` in the component, which also covers SSR |
...
| A keyboard user tabs through invisible duplicate links, or a `getByRole` query finds two of something the page shows once | The desktop nav and the mobile panel are both always in the DOM and only CSS hides one, so the hidden copy stays in the accessibility tree and the tab order | Put `inert` + `aria-hidden` on whichever copy is currently hidden. `MobileNav.test.tsx` asserts exactly one `About` link exists while the panel is closed |
| `queryByRole('link', { current: 'true' })` matches nothing, so an assertion about the active tab passes whether or not a tab is marked | Testing Library normalises `aria-current` to a boolean, and the string `'true'` never matches — a false-negative query that reads as a passing test | Query `{ current: true }`. Pair it with a positive-control case asserting the active tab IS found, or the query can silently stop discriminating again |
